...
logging.basicConfig(
    filename='build/logs/etl_process_ocr.log',
    level=logging.INFO,
...
class InvoiceProcessor:
    def __init__(self, output_dir="build/output/faturas_processadas_ocr"):
        ...
        self.md_output_dir = "build/output/faturas_md_ocr"
    ...
    def process_pdf(self, pdf_path, use_ocr=False):
        ...
        logging.info(f"Iniciando processamento (OCR): {filename}")
                        ...
                        if ("lancamentosprodutoseservicos" in line_norm or "lançamentosprodutoseserviços" in line_norm):
                            ...
                            if mv:
                                ...
                                already_ps = any(
                                    ...
                                    if t.get("final_cartao") == current_card_number and t.get("estabelecimento") == "Produtos e serviços"
                                )
                                if not already_ps:
                                    transactions.append({
                                        "arquivo": filename,
...
def run_etl():
    ...
    processor = InvoiceProcessor(output_dir="build/output/faturas_processadas_ocr")
    ...
    print(f"Iniciando processamento OCR de {len(files)} arquivos...")
    ...
    with open("build/logs/VALIDATION_REPORT_OCR.md", "w") as report:
        report.write("# Relatório de Validação do ETL (OCR)\n\n")
    ...
    if not master_df.empty:
        master_df.to_csv("build/output/faturas_consolidado_ocr.csv", index=False)
        
        conn = sqlite3.connect("build/db/faturas_ocr.db")
        ...
        print("Relatório de validação gerado: build/logs/VALIDATION_REPORT_OCR.md")
        print("Base consolidada atualizada: build/db/faturas_ocr.db")
        ...
        try:
            ...
            if debug_rows:
                pd.DataFrame(debug_rows).to_csv('build/logs/debug_discrepancias_ocr.csv', index=False)
                print('Debug gerado: build/logs/debug_discrepancias_ocr.csv')
        except Exception as e:
            logging.debug(f'Falha ao gerar debug_discrepancias_ocr.csv: {e}')
    else:
        print("Nenhum dado foi extraído (OCR).")
